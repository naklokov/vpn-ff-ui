...
const isMaskedPhoneEmpty = (value: string): boolean =>
...
export const registerFormSchema = z
  .object({
    ...
    // Необязательное поле: пустое значение допустимо.
    referralUserLogin: z
      .string()
      .refine(
        (v) => isMaskedPhoneEmpty(v) || isValidRuMobile(v),
        "Нужен мобильный номер в формате +7*********",
      ),
  })
...
export const paymentFormSchema = z.object({
  period: z.coerce
    ...
    .refine(
      (v) => v === 1 || v === 3 || v === 6,
      "Доступны только 1, 3 или 6 месяцев",
    ),
  phone: z
    .string()
    .min(1, "Укажите номер телефона")
